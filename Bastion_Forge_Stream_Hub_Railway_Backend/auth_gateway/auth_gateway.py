    ...
    if session is None:
        return HTMLResponse(
            (
                ...
                "<p>Login session was not found or has expired.</p>"
            ),
            status_code=400,
        )
...
async def oauth_status(
    ...
    data = {
        "status": "complete",
        "access_token": session.token_data.get(
            "access_token"
        ),
        "refresh_token": session.token_data.get(
            "refresh_token"
        ),
        "client_id": session.token_data.get(
            "client_id"
        ),
    }

    # One-time token delivery.
    # Once Stream Hub retrieves the completed
    # login session, remove it from memory.
    sessions.pop(
        session_id,
        None,
    )
